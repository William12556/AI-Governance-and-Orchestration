"""
Tests for engine orchestrator behaviours listed as unexercised in dev/backlog.md
...
ENGINE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "ai", "engine"))
...
def test_t1_worker_summary_kept_on_budget_exhaustion(orch, log, project):
    summary = project / "ai" / "state" / "work-summary.txt"
    script = [
...
def test_t2_move_file_destination_recorded(orch, log, project):
    ...
    summary = (project / "ai" / "state" / "work-summary.txt").read_text()
    assert rc == 0
...
def test_t3_log_archive_unset_is_noop(orch, project):
    ...
    (state / "engine_20260101-000000.LOG").write_text("log\n")
...
def test_t5_blocked_after_work_phase_exits(orch, log, project, monkeypatch):
    def worker(state_dir, n):
        with open(os.path.join(state_dir, "BLOCKED.md"), "w") as fh:
            fh.write("# BLOCKED\n\ntest\n")
        return 0, "", set()
...
def test_t9_stall_detection_blocks(orch, log, project, monkeypatch):
    ...
    blocked = (project / "ai" / "state" / "BLOCKED.md").read_text()
...
def test_d1_mcp_bounded_below_2():
    with open(os.path.join(ENGINE_DIR, "requirements.txt")) as fh:
