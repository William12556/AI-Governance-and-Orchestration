...
2. Propagate governance v10.x to GTach, solax-modbus, e-Paper-IP-Display, pi-netconfig and certmon; pinned at v9.16 by decision D5. `bin/propagate.sh` fixed under triple 07087e91; first run requires `--allow-major` (9.16 → 10.x). solax-modbus propagated 2026-09-23 (9.11 → 10.2). Blocked until the final re-check of change-b170cf6a iteration 2 and the macOS procedure in `dev/audit/closed/audit-c5270084-followup-2026-09-23.md` §8.0 (plus `ai/Templates/` and `ai/Context.md` cases). Stop rule: close if no finding of medium or higher. The script never deletes: retired and project files go to `ai-local/`; delete after review (governance P10.6).
