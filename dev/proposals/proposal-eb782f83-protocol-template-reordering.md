...
The snapshot is git-tracked and retained until the coupled change document is
closed and the audit reports clean. It is not deleted without explicit
instruction.

Note: the snapshot duplicates information already held in git history. It is
retained as an explicitly required, out-of-band mechanism that does not depend
on git integrity.
